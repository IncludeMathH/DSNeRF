...
def render(H, W, focal, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    if c2w is not None:
        ...
        rays_o, rays_d = get_rays(H, W, focal, c2w, device=kwargs['device'])
    else:
    ...
    if use_viewdirs:
        ...
        if c2w_staticcam is not None:
            ...
            rays_o, rays_d = get_rays(H, W, focal, c2w_staticcam, device=kwargs['device'])
...
def render_test_ray(rays_o, rays_d, hwf, ndc, near, far, use_viewdirs, N_samples, network, network_query_fn, **kwargs):
    ...
    rgb, sigma, depth_maps, weights = sample_sigma(rays_o, rays_d, viewdirs, network, z_vals, network_query_fn, device=kwargs['device'])
...
def create_nerf(args):
    ...
    if args.alpha_model_path is None:
        ...
        elif args.model_type == 'VimCm':
            model = NeRF_VimCm(D=args.netdepth, W=args.netwidth,
                        input_ch=input_ch, output_ch=output_ch,
                        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs,
                        device=device,
                        ).to(device)
    ...
    if args.N_importance > 0:
        if args.alpha_model_path is None:
            if args.model_type == 'v1':
                ...
                model_fine = NeRF_VimCm(D=args.netdepth, W=args.netwidth,
                            input_ch=input_ch, output_ch=output_ch,
                            input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs,
                            device=device,
                            ).to(device)
...
#     raw = run_network(pts)
    ...
    if N_importance > 0:
        ...
        z_samples = sample_pdf(z_vals_mid, weights[...,1:-1], N_importance, det=(perturb==0.), pytest=pytest, device=device)
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            if N_rand is not None:
                rays_o, rays_d = get_rays(H, W, focal, torch.Tensor(pose), device=device)  # (H, W, 3), (H, W, 3)
